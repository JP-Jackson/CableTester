...
- **A capability granted to a service is not granted to your shell.** `deploy/cabletester.service` has `AmbientCapabilities=CAP_NET_ADMIN` so the ethernet ladder can reconfigure an interface. Running the same code from a prompt gets none of that: use `sudo .venv/bin/python run.py --eth-test A B`. And the unit file is a copy, so a `git pull` alone never updates it; `setup-pi.sh` does.
